...
- **タイムライン** — ガント風のタイムライン。タスク/イベントバーはドラッグで日付をスライド・リサイズでき、グループ枠ごとまとめて移動もできる。タスク同士の依存関係は矢印で表示される
...
- **フロントエンド** — React + TypeScript + Vite / UI は shadcn/ui(Tailwind CSS v4)/ タイムラインは [@xyflow/react](https://reactflow.dev/)(React Flow)ベース
...
- **tasks** — プロジェクトに所属(必須)、グループへの所属は任意。ステータス / 優先度 / 開始・終了日 / タイムライン上の座標を持つ
